...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setDiscountPrice(data.discountPrice > 0 ? data.discountPrice.toString() : '');
          setSizes(data.sizes || []);
  ...
  if (isFetching) {
    return <div className="max-w-5xl mx-auto flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#881337]" /></div>;
  }
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-3xl font-bold text-[#1c1917]">Edit Product</h1>
          </div>
        </div>
